        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12'>
            {exclusiveOffers.map((item) => (
                ...
                <div>
                    ...
                    <p className='tex-xs text-white/70 mt-3'>Expires {item.expiryDate}</p>
                </div>
                <button className='flex items-center gap-2 font-medium cursor-pointer mt-4 mb-5'>
                    View offers
                    <img className='inver group-hover:translate-x-1 transition-all' src={assets.arrowIcon} alt="arrow-icon" />
                </button>
                </div>
            ))}
        </div>
